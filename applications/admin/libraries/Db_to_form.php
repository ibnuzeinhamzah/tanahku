...
class Db_To_Form 
{
	...
	*/
	function create_list_view($mod, $modvars, $data, $pdf = false)
	{
		$field = $modvars->field;
		$view = '';
		$view .= '<div class="box box-solid">';
		$view .= '<div class="box-header with-border">';
		if(isset($modvars->insertable)){
			if(!$modvars->insertable) $view .= '';
			else $view .= '<div><a href="'.base_url().'admin/'.$mod.'/insert"><i class="fa fa-fw fa-plus-circle"></i> Insert '.$modvars->name.'</a></div>';
		}else{
			$view .= '<div><a href="'.base_url().'admin/'.$mod.'/insert"><i class="fa fa-fw fa-plus-circle"></i> Insert '.$modvars->name.'</a></div>';
		}
		$view .= '</div>';

		$view .= '<div class="box-body">';		
		$view .= $this->header_admin_table($field);

		for($i=0;$i<count($data);$i++)
		{
			$url_detail = '<a href="'.base_url().'admin/'.$mod.'/detail/'.$data[$i]->id.'">';
			$url_edit = '<a href="'.base_url().'admin/'.$mod.'/edit/'.$data[$i]->id.'"><i class="glyphicon glyphicon-edit"></i></a>';
			$url_delete = '<a href="'.base_url().'admin/'.$mod.'/confirm-delete/'.$data[$i]->id.'"><i class="glyphicon glyphicon-remove-circle glyphicon-danger"></i></a>';
			$url_pdf = ($pdf) ? '<a href="'.base_url().'admin/drh/pdf/'.$data[$i]->user_id.'">PDF</a>' : '';
			$view .= '<tr>';
			...
			$view .= '<td>'.$url_edit.' &nbsp; '.$url_delete.' &nbsp; '.$url_pdf.'</td>';
			$view .= '</tr>';
		}
	...
	function create_detail_view($mod, $modvars, $data)
	{
		...
		$view .= '<div class="box-header with-border">';
		$view .= '<div class="row">';
		$view .= '<div class="col-xs-7"><a href="'.base_url().'admin/'.$mod.'"><i class="fa fa-fw fa-bars"></i> Back to list</a></div>';
		$view .= '<div class="col-xs-5 pull-right">';
		// $view .= '<div class="col-xs-6"><a href="'.base_url().'admin/'.$mod.'/insert"><i class="fa fa-fw fa-plus-circle"></i> Insert '.$modvars->name.'</a></div>';
		$view .= '<div class="col-xs-3 pull-right"><a href="'.base_url().'admin/'.$mod.'/edit/'.$data[0]->id.'"><i class="fa fa-fw fa-edit"></i> &nbsp;Edit</a></div>';
		// $view .= '<div class="col-xs-3 pull-right"><a href="'.base_url().'admin/'.$mod.'/confirm-delete/'.$data[0]->id.'"><i class="fa fa-fw fa-remove"></i> &nbsp;Delete</a></div>';
		$view .= '</div>';
		$view .= '</div>';
		$view .= '</div>';
