...
class Drh
{
    ...
    function pdf_form()
    {
        ...
        $registrasi = $CI->admin_model->get_data_by_id('pendaftar',$user_id,'user_id');
        $biodata = $CI->admin_model->get_data_by_id('biodata',$user_id,'user_id');
        $pendidikan = $CI->admin_model->get_data_by_sql('SELECT tingkat_pendidikan, jurusan, universitas, ipk FROM riwayat_pendidikan WHERE user_id = '.$user_id.' ORDER BY tingkat_pendidikan DESC');
        $diklat = $CI->admin_model->get_data_by_sql('SELECT * FROM riwayat_diklat WHERE user_id='.$user_id.' ORDER BY tahun DESC');
        $jabatan = $CI->admin_model->get_data_by_id('riwayat_jabatan',$user_id,'user_id');
        $pangkat = $CI->admin_model->get_data_by_sql('SELECT tmt, golongan FROM riwayat_pangkat WHERE user_id='.$user_id.' ORDER BY tmt DESC');
        $penugasan = $CI->admin_model->get_data_by_sql('SELECT * FROM riwayat_penugasan WHERE user_id='.$user_id.' ORDER BY tahun_awal DESC');
        $prestasi = $CI->admin_model->get_data_by_sql('SELECT * FROM riwayat_prestasi WHERE user_id='.$user_id.' ORDER BY tahun DESC');
        $karya_tulis = $CI->admin_model->get_data_by_sql('SELECT * FROM karya_tulis WHERE user_id='.$user_id.' ORDER BY tahun DESC');
        $agama = $CI->admin_model->get_data_by_id('agama',$biodata[0]->agama,'id');
        $formasi = $CI->admin_model->get_data_by_id('formasi_jabatan',$registrasi[0]->jurusan_formasi,'id');

        $instansi = $CI->admin_model->get_data_by_sql('SELECT instansi FROM riwayat_jabatan WHERE user_id = '.$user_id.' ORDER BY tmt DESC LIMIT 1');

        $foto = $CI->admin_model->get_data_by_sql('SELECT document FROM dokumen_pendukung WHERE user_id='.$user_id.' AND jenis_document=4 LIMIT 1');
        ...
        foreach($pendidikan as $p) {
            $tingkat_pendidikan = $CI->admin_model->get_data_by_id('tingkat_pendidikan',$p->tingkat_pendidikan,'id');
        ...
        foreach($pangkat as $p) {
            $gol = $CI->admin_model->get_data_by_id('tbl_pangkat',$p->golongan,'id');
