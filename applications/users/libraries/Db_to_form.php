...
class Db_To_Form 
{
	...
	*/
	function create_list_view($mod, $modvars, $data)
	{
		...
		if($this->CI->editable()){
			$view .= (isset($modvars->maxdata) && count($data) >= $modvars->maxdata) ? '' : '<div><a href="'.base_url().'users/'.$mod.'/insert"><i class="fa fa-fw fa-plus-circle"></i> Insert '.$modvars->name.'</a></div>';
		}
		$view .= '</div>';

		$view .= '<div class="box-body">';		
		$view .= $this->header_users_table($field);

		for($i=0;$i<count($data);$i++)
		{
			$url_detail = '<a href="'.base_url().'users/'.$mod.'/detail/'.$data[$i]->id.'">';
			$url_edit = '<a href="'.base_url().'users/'.$mod.'/edit/'.$data[$i]->id.'"><i class="glyphicon glyphicon-edit"></i></a>';
			$url_delete = '<a href="'.base_url().'users/'.$mod.'/confirm-delete/'.$data[$i]->id.'"><i class="glyphicon glyphicon-remove-circle glyphicon-danger"></i></a>';
			$view .= '<tr>';
			...
			if($this->CI->editable()){ $view .= '<td>'.$url_edit.' &nbsp; '.$url_delete.'</td>'; }
			$view .= '</tr>';
		}
	...
	function create_detail_view($mod, $modvars, $data)
	{
		...
		$view .= '<div class="box-header with-border">';
		$view .= '<div class="row">';
		$view .= '<div class="col-md-12"><a href="'.base_url().'users/'.$mod.'"><i class="fa fa-fw fa-bars"></i> &nbsp;</a>';
		if($this->CI->editable()){
			$view .= (isset($modvars->maxdata) && count($data) >= $modvars->maxdata) ? '' : '<a href="'.base_url().'users/'.$mod.'/insert"><i class="fa fa-fw fa-plus-circle"></i> </a>';
			$view .= '<a href="'.base_url().'users/'.$mod.'/edit/'.$data[0]->id.'"><i class="fa fa-fw fa-edit"></i> &nbsp;</a>';
			$view .= '<a href="'.base_url().'users/'.$mod.'/confirm-delete/'.$data[0]->id.'"><i class="fa fa-fw fa-remove"></i> &nbsp;</a>';
		}
		$view .= '</div>';
		$view .= '</div>';
		$view .= '</div>';
	...
	function create_insert_view($mod, $modvars)
	{
		...
		$view .= '<div class="col-lg-12">';
		$view .= '<div class="box box-solid">';
		$view .= '<div class="box-header with-border"><i class="fa fa-fw fa-pencil"></i> <h3 class="box-title"> '.$modvars->name.'</h3></div>';

		$view .= '<form role="form" method="post" action="'.base_url().'users/'.$mod.'/save-insert" enctype="multipart/form-data">';
		$view .= '<div class="box-body">';
		foreach ($field as $k => $v) {
			if($v['type'] != 'primary_key')
			{
				...
				$view .= '<dl class="dl-horizontal">';
				$view .= '<dt><label>'.$text.'</label></dt>';
				$view .= '<dd>'.$this->generate_field_form($v,$k).'</dd>';
				$view .= '</dl>';
			}
		}

		if(isset($modvars->ket)) $view .= $modvars->ket;
		$view .= '</div>';
        ...
        $view .= '<button type="reset" class="btn btn-sm btn-flat btn-default" onclick="location.href=\''.base_url().'users/'.$mod.'\'">Cancel</button>';
      	$view .= '</div>';
		$view .= '</form>';
	...
	function create_edit_view($mod, $modvars, $data)
	{
		...
		$view .= '<div class="col-lg-12">';
		$view .= '<div class="box box-solid">';
		$view .= '<div class="box-header with-border"><i class="fa fa-fw fa-pencil"></i> <h3 class="box-title">Edit '.$modvars->name.'</h3></div>';

		$view .= '<form role="form" method="post" action="'.base_url().'users/'.$mod.'/save-update" enctype="multipart/form-data">';
		$view .= '<div class="box-body">';
		foreach ($field as $k => $v) {
			if($v['type'] != 'primary_key')
			{
				...
				$view .= '<dl class="dl-horizontal">';
				$view .= '<dt><label>'.$text.'</label></dt>';
				$view .= '<dd>'.$this->generate_field_form($v,$k,$value).'</dd>';
				$view .= '</dl>';
			}
		}
		$view .= '</div>';
	...
	function create_delete_view($mod, $modvars, $data)
	{
		...
		$view .= '<form role="form" method="post" action="'.base_url().'users/'.$mod.'/delete/'.$data[0]->id.'" enctype="multipart/form-data">';
		$view .= '<div class="box-header with-border">';
        ...
        $view .= '<button type="reset" class="btn btn-sm btn-flat btn-primary" onclick="location.href=\''.base_url().'users/'.$mod.'\'">Cancel</button>';
        $view .= '</div>'; // col-xs-10
		//$view .= '<div class="col-xs-2 pull-right"><h5><a href="'.base_url().'users/'.$mod.'"><i class="fa fa-fw fa-bars"></i> Back to list</a></h5></div>';
		$view .= '</div>'; // box-header
	...
	function header_table($field)
	{
		...
		foreach ($field as $key => $value) {
			...
			$header .= '<th '.$width.'>'.ucwords($key).'</th>'; 
		}
	...
	function header_users_table($field)
	{
		$header = '<table class="table table-bordered table-striped" id="list-data">';
		$header .= '<thead>';
		$header .= '<tr>';
		...
		if($this->CI->editable()){ $header .= '<th width="100px">Edit / Delete</th>'; }
		$header .= '</tr>';
		$header .= '</thead>';
	...
	function generate_text_value($field, $field_name, $value = "") { return '<div>'.$value.'</div>'; }
	...
	function get_fkey_form($field, $fieldname, $value = ""){
		$CI =& get_instance();
		include_once APPPATH.'/controllers/Users/'.$field['fkey'].'.php';
		$modvars = new $field['fkey'];
		$modtable = $modvars->table;
		$fkey = $modvars->fkey;
		$data = $CI->users_model->get_all_data($modtable);
		if(isset($field['readonly']) && $field['readonly'] == true){
			if($value == ""){
				$value = $CI->session->user_id;
				//$v = $this->generate_text_form($field, $fieldname, $value);
			}
			$v = '<select name="'.$fieldname.'" class="form-control input-sm" readonly="readonly">';
			for($i=0;$i<count($data);$i++){ 
				if($value == $data[$i]->$fkey['value']){
	...
	*/
	...
	function get_fkey_value($field, $fieldname, $value = ""){
		...
		$modtable = $modvars->table;
		$fkey = $modvars->fkey;
		$data = $CI->users_model->get_data_by_id($modtable,$value,$fkey['value']);
		$v = '<div>';
		if(is_array($fkey['text'])){
	...
	*/
	...
	function generate_date_form($field, $field_name, $value = "")
	{
		...
		$forms .= '<input type="text" class="form-control input-sm date" name="'.$field_name.'" id="'.$field_name.'" ';
		if(isset($field['readonly']) && $field['readonly'] == true) $forms .= 'readonly="readonly" ';
	...
	function generate_date_value($field, $field_name, $value = "") { return '<div>'.$value.'</div>'; }

	function generate_year_form($field, $field_name, $value = "")
	{
		$forms = '<div class="input-group">';
		$forms .= '<div class="input-group-addon"><i class="fa fa-calendar"></i></div>';
        $forms .= '<input type="text" class="form-control year input-sm" ';
        if(isset($field['readonly']) && $field['readonly'] == true) $forms .= 'readonly="readonly" ';
	...
	function generate_file_value($field, $field_name, $value = "") { 
		return ($value == "") ? "" : '<div><a href="'.base_url().$field['dir_to_save'].'/'.$value.'">'.$value.'</a></div>'; 
	}
	...
	function generate_flag_value($field, $field_name, $value = "") { 
		$rvalue = "";
		if($value != ""){
			if(isset($field['flag_value'][$value])) $rvalue = '<div>'.$field['flag_value'][$value].'</div>';
		}
	...
	function get_parent_data($field, $parent_table, $parent_field, $fields_to_show, $value = "")
	{
		...
		return $CI->usersmod->get_data($sql);
	}
	...
	function do_upload_foto($table, $fieldname, $field, $files, $id = "", $content)
	{
		...
		if(is_array($files) && isset($files[$fieldname]))
		{
			...
			$CI->usersmod->update($table, 'id', $id, $data);
		}
		else
		{
			if($files['error'][0] == 0)
	...
	function do_upload($table, $fieldname, $field, $files, $id = "")
	{
		$CI =& get_instance();
		if(isset($field['dir_to_save'])) $uploaddir = $field['dir_to_save'] . '/';
		else $uploaddir = FILE_DIR . '/';
		$error = array();
		...
		else
		{
			if($files['error'] == UPLOAD_ERR_OK)
			{
				...
				if (move_uploaded_file($files['tmp_name'], $uploadfile)) { 
					if($CI->uri->rsegment(2) != "save_json"){
						$data[$fieldname] = $id."_".$this->safe_str(basename($files['name']));
						$CI->usersmod->update($table, 'id', $id, $data);
					}
				} 
				else { $error[$key] = 0; }
	...
	function generate_album_dir($id)
	{
		return mkdir(ALBUM_DIR . '/' . $id, 0774);
	}

	function generate_static_pages_form($field, $field_name, $value = "")
	{
		$contents = "";
		if($value != "")
		{
			$filename = APPPATH.STATIC_PAGES_DIR . '/' . $value;
		...
		$forms = $this->generate_text_form($field, $field_name, $value);
		$forms .= '<br/><br/>';

		$forms .= '<div class="col-xs-10">';
		$forms .= '<textarea name="content_'.$field_name.'" id="textArea" ';
		if(isset($field['readonly']) && $field['readonly'] == true) $forms .= 'readonly="readonly" ';
		$forms .= 'class="form-control" rows="12" placeholder="Enter text..">'.$contents.'</textarea>';
		$forms .= '</div>';
		return $forms; 
	}

	function generate_static_pages_value($field, $field_name, $value = "") 
	{ 
		$contents = "";
		if($value != "")
		{
		...
		$forms = $this->generate_text_value($field, $field_name, $value);
		$forms .= '<div>'.$contents.'</div>'; 
		return $forms;
	}

	function write_static_pages($filename, $content)
	{
		$filedir = realpath(APPPATH.STATIC_PAGES_DIR);
